...
export function Products() {
  ...
  const hearbalCategoryOptions = ['mv-hearbal', 'beauty', 'health'].map((slug) => [slug, categoryLabels.get(slug) || (slug === 'mv-hearbal' ? 'All Mv Hearbal' : slug)]).filter(([, name]) => name);
    ...
    : category === 'tolip' ? ['Tolip']
      : category === 'health-tolip' ? ['Tolip', 'Health']
        : categoryDescendants['health-tolip'].includes(category) ? ['Tolip', 'Health', activeCategoryLabel]
          : (category === 'hearbal' || category === 'mv-hearbal') ? ['Mv Hearbal']
            : category === 'beauty' ? ['Mv Hearbal', 'Beauty']
              : category === 'health' ? ['Mv Hearbal', 'Health']
      ...
      <section className="catalog shell">
        ...
        <div className="catalog-layout">
          <aside id="catalog-filters" className={`catalog-filters ${filtersOpen ? 'is-open' : ''}`}>
            ...
            <div className="filter-group"><label htmlFor="category-filter">Brand & category</label><select id="category-filter" value={category} onChange={(event) => chooseCategory(event.target.value)}><option value="all">All products</option><optgroup label="Tolip">{tolipCategoryOptions.map(([slug, name]) => <option key={slug} value={slug}>{slug === 'tolip' ? 'All Tolip' : slug === 'health-tolip' ? 'All Health' : name}</option>)}</optgroup><optgroup label="Mv Hearbal">{hearbalCategoryOptions.map(([slug, name]) => <option key={slug} value={slug}>{slug === 'hearbal' || slug === 'mv-hearbal' ? 'All Mv Hearbal' : name.replace(/^(Mv )?Hearbal \/ /, '')}</option>)}</optgroup>{otherCategoryOptions.length > 0 && <optgroup label="Other wellness categories">{otherCategoryOptions.map(([slug, name]) => <option key={slug} value={slug}>{name}</option>)}</optgroup>}</select></div>
